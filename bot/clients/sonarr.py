...
class SonarrClient(ArrBaseClient):
    ...
    async def add_series(
        ...
        Args:
            ...
            monitor: Monitor type (all, future, missing, existing, pilot, firstSeason, latestSeason, none)
            search_for_missing: Whether to search for missing episodes
            series_type: Sonarr seriesType ("standard" or "anime")
        ...
        # Build seasons with monitoring based on monitor
        seasons = []
        if series.seasons:
            for s in series.seasons:
                season_num = s.get("seasonNumber", 0)
                season_monitored = self._should_monitor_season(season_num, monitor, series.season_count)
                seasons.append({
        ...
        payload = {
            ...
            # Rollback 2026-08-10: anime is a seriesType, not a separate
            # library — absolute episode numbering depends on this field.
    ...
    # inherited from ArrBaseClient (r5 follow-up: ArrBaseClient dedup) — Sonarr
