...
> ✅ **Реализовано (TDD, 383 passed, коммиты c292e96/e44f81d):** #5 внешние ссылки, #7 `/health`,
> #2 пресеты мониторинга сезонов, #4 pre-grab проверка «уже в Emby».
> ⏳ **Осталось (крупные/архитектурные, требуют решения):** #1 типизированный CallbackData (рефакторинг),
> #3 FSM-мастер, #8 webhook-уведомления (нужен inbound-порт), #6 `/users` runtime (затрагивает auth).
