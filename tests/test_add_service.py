...
async def test_validate_download_url_trusts_a_configured_service_host(monkeypatch):
    ...
    monkeypatch.setenv("SCRYER_URL", "http://192.168.31.95:8088")
    from bot.config import get_settings

    get_settings.cache_clear()
    try:
        assert await _validate_download_url("http://192.168.31.95:8088/download?x=1") is True
        assert await _validate_download_url("http://192.168.31.95:22/") is False
    finally:
        get_settings.cache_clear()
