...
### 6. Navidrome: учётные данные приходят только от владельца

Пароль пользователя Navidrome лежит в его БД (`C:\ProgramData\Navidrome\navidrome.db`),
в `navidrome.ini` его нет — поэтому клиент включается только после добавления
`NAVIDROME_USERNAME` / `NAVIDROME_PASSWORD` в `.env`. Без них фича молча выключена
(`navidrome_enabled == False`), а при старте выдаётся предупреждение о неполной настройке.

Учётные данные получены от владельца 2026-07-29 и прописаны на RPi4. Проверено в проде:
`ping` → Navidrome 0.61.1; «Amatory / DOOM» (есть в библиотеке) → «✅ Уже есть в библиотеке»,
«Metallica / Master of Puppets» (нет) → подсказки нет.
...
## Наблюдение: Scryer сам душит индексеры

При интенсивных прогонах (несколько подряд live-smoke + ручные проверки) Scryer пишет
`indexer backoff escalated indexer="RuTracker.org" disabled_until=… escalation_level=1`
и `searchReleases` начинает возвращать **пустой список за ~400 мс** — то есть даже не
опрашивает индексеры, хотя Prowlarr напрямую в тот же момент отдаёт 10 результатов.
Через ~5 минут backoff истекает и поиск возвращается к норме (проверено: 23:33 → 0 релизов
за 429 мс, 23:36 → 5 релизов за 3.3 с).

Это не дефект бота и не регресс миграции — это защита Scryer от превышения лимитов трекера.
Практический вывод для эксплуатации: серия быстрых поисков подряд «съедает» окно, и
следующий поиск вернёт пусто. Бот в этом случае честно показывает «Релизы не найдены».
