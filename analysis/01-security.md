...
- **Проблема**: контейнер запускается как `botuser` UID 1000. `bot-data:/app/data` mount-ится как named volume — права создаёт docker, обычно `1000:1000`. Если хост-пользователь user имеет UID ≠ 1000, прямой доступ к sqlite-файлу может быть ограничен/расширен непредсказуемо. На Linux хосте без ACL bot.db (содержит сессии/историю) доступен любому, кто читает named-volume. Для homelab — ОК, но не documented.
- **Риск**: Низкий
