...
class BaseAPIClient:
    ...
    async def _request(
        ...
        try:
            ...
            raise APIConnectionError(f"Таймаут соединения с {self.service_name}") from e

        except httpx.ConnectError as e:
            log.error("Connection error", error=str(e))
            raise APIConnectionError(f"Не удалось подключиться к {self.service_name} ({self.base_url})") from e
